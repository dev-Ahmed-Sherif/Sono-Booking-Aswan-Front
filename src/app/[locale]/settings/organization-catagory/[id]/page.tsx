...
import OrganizationCatagoryForm from "@/components/settings/organization-catagory/organization-catagory-form";

type PageProps = {
  params: { locale: string; id: string };
};

const OrganizationCatagoryPage = async ({ params }: PageProps) => {
  const { id } = params;
  let initialData: unknown = null;
  if (id && id !== "new") {
    const result = await getOrganizationCategoryById(id);
