...
type RoomTypesPageProps = { params: { locale: string } };

const RoomTypesPage = async ({ params }: RoomTypesPageProps) => {
  const { locale } = params;
  let data: RoomTypeColumn[] | null = null;
  const result = await getRoomTypes();
  if (result && !(result as { error?: string }).error) {
    const raw = (result as { data?: unknown }).data ?? result;
    data = Array.isArray(raw) ? (raw as RoomTypeColumn[]) : null;
  }
  return (
