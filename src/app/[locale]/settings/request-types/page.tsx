...
type RequestTypesPageProps = { params: { locale: string } };

const RequestTypesPage = async ({ params }: RequestTypesPageProps) => {
  const { locale } = params;
  let data: RequestTypeColumn[] | null = null;
  const result = await getRequestTypes();
  if (result && !(result as { error?: string }).error) {
    const raw = (result as { data?: unknown }).data ?? result;
    data = Array.isArray(raw) ? (raw as RequestTypeColumn[]) : null;
  }
  return (
