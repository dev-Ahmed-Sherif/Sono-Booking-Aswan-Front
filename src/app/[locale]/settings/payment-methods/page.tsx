...
type PaymentMethodsPageProps = { params: { locale: string } };

const PaymentMethodsPage = async ({ params }: PaymentMethodsPageProps) => {
  const { locale } = params;
  let data: PaymentMethodColumn[] | null = null;
  const result = await getPaymentMethods();
  if (result && !(result as { error?: string }).error) {
    const raw = (result as { data?: unknown }).data ?? result;
    data = Array.isArray(raw) ? (raw as PaymentMethodColumn[]) : null;
  }
  return (
