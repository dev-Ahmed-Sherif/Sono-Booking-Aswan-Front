import { getMaintenanceTypeById } from "@/actions/settings/maintenanceTypeService";
import MaintenanceTypesForm from "@/components/settings/maintenance-types/maintenance-types-form";

type PageProps = {
  params: { locale: string; id: string };
};

const MaintenanceTypePage = async ({ params }: PageProps) => {
  const { id } = params;
  let initialData: unknown = null;
  if (id && id !== "new") {
    const result = await getMaintenanceTypeById(id);
