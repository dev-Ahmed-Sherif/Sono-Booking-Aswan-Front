...
import {
  InspectionFloatingUnitClauseColumn,
  createColumns,
} from "@/components/inspections/inspection-floating-unit-clause/columns";
...
type ClientProps = {
  data: InspectionFloatingUnitClauseColumn[] | null;
  path: string;
  backPath?: string;
...
const Client = memo(
  ({
    ...
    path,
    backPath,
    editPathPrefix = "/inspections/inspection-floating-unit-clause",
  }: ClientProps) => {
    const params = useParams();
    const router = useRouter();
    const user = useLocalStorage("user");
    const locale = (params?.locale as string) ?? "ar";
    const resolvedBackPath = backPath ?? `/${locale}/inspections`;
    const isSuperAdmin = useMemo(
      () => isSuperAdminRoleCandidates(user.getItem() as RoleCandidates),
      [user],
    );
    const columns = createColumns(isSuperAdmin, editPathPrefix);
    ...
    return (
      <div className="mt-7 sm:mt-0 space-y-4">
        ...
        <div className="p-2 sm:p-4 flex flex-col sm:flex-row items-center sm:justify-between gap-3 sm:gap-4">
          <div className="w-full sm:w-auto text-center sm:text-right">
            <Heading
              ...
              description="إدارة بنود تفتيش الوحدات العائمة"
            />
          </div>
          <Button
        ...
        <div className="overflow-x-auto rounded-md">
          ...
          <DataTable
            searchKey="inspectionClauseName"
            columns={columns}
            data={tableData}
          />
        </div>
