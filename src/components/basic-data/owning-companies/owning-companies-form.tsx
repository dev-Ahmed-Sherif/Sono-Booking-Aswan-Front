...
import { owningCompaniesSchema } from "@/schemas";
import {
  addOwningCompany,
  deleteOwningCompanyById,
  softDeleteOwningCompanyById,
  updateOwningCompanyById,
...
type FormValues = Omit<
  z.infer<typeof owningCompaniesSchema>,
...
function buildOwningCompanyFormData(
  values: FormValues,
  options?: { id?: string; organizationCategoryId?: string },
): FormData {
  const fd = new FormData();
  if (options?.id) {
    fd.append("id", options.id);
  }
  if (options?.organizationCategoryId) {
    fd.append("organizationCategoryId", options.organizationCategoryId);
  }
...
const OwningCompaniesForm = ({
  initialData,
  name,
  organizationCategoryId: organizationCategoryIdProp,
  ...
  const categoryId =
    (params.categoryId as string) || organizationCategoryIdProp || "";
  ...
  const validationSchema = useMemo(() => {
    ...
    const categoryRaw = organizationCategoryName?.trim() ?? "";
    const isTouristMarinaCategory =
      categoryRaw.includes("مراسى") ||
      categoryRaw.includes("مراسي") ||
      categoryRaw.toLowerCase().includes("marina");
    const isFloatingRequirementsCategory =
      categoryRaw.includes("عائمات") ||
      categoryRaw.includes("عائمة") ||
      categoryRaw.toLowerCase().includes("floating units") ||
      categoryRaw.toLowerCase().includes("floatingunit") ||
      categoryRaw.toLowerCase().replace(/\s+/g, "").includes("floatingunits");

    return owningCompaniesSchema
      .omit({
        commercialRegistrationAttachment: true,
        touristMarinaNumber: true,
        isAccepted: true,
      })
      .extend({
        commercialRegistrationAttachment: z.any().optional(),
        touristMarinaNumber: z.string().optional(),
      ...
      .superRefine((vals, ctx) => {
        ...
        const marinaNumber = (vals.touristMarinaNumber ?? "").trim();
        if (isTouristMarinaCategory) {
          if (!marinaNumber) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message: "عدد المراسي السياحي مطلوب",
              path: ["touristMarinaNumber"],
            });
          } else if (!/^\d+$/.test(marinaNumber)) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message: "عدد المراسي السياحي يجب أن يكون أرقاماً فقط",
              path: ["touristMarinaNumber"],
            });
          } else if (marinaNumber.length >= 3) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
        ...
        if (isFloatingRequirementsCategory && vals.isAccepted !== true) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
    ...
    initialData,
    initialCommercialPath,
    existingAttachmentRemoved,
    organizationCategoryName,
  ]);

  const locale = (params?.locale as string) ?? "ar";
  const listBackPath = categoryId
    ? `/${locale}/basic-data/owning-companies/${categoryId}`
    : `/${locale}/basic-data/owning-companies`;
  ...
  const showTouristMarinasSection = useMemo(() => {
    const raw = organizationCategoryName?.trim();
    if (!raw) return false;
    const lower = raw.toLowerCase();
    return (
      raw.includes("مراسى") || raw.includes("مراسي") || lower.includes("marina")
    );
  }, [organizationCategoryName]);
  const showFloatingRequirementsSection = useMemo(() => {
    const raw = organizationCategoryName?.trim();
    if (!raw) return false;
    const lower = raw.toLowerCase();
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      touristMarinaNumber: data?.touristMarinaNumber || "",
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      let savedId = "";
      if (data?.id) {
        const res = (await updateOwningCompanyById(
          buildOwningCompanyFormData(values, {
            id: data.id,
        ...
        const res = (await addOwningCompany(
          buildOwningCompanyFormData(values, {
            organizationCategoryId: categoryId,
          }),
        )) as SaveResponse;
  ...
  const syncOwningCompanyAfterEmployeeChange = async () => {
    if (!data?.id) return;
    try {
  ...
  const onCompleteEmployees = () => {
    ...
    if (pendingSavedId && categoryId) {
      router.push(
        `/${locale}/basic-data/owning-companies/${categoryId}/${pendingSavedId}`,
      );
      setPendingSavedId("");
      return;
    }
            ...
            >
              <div className="space-y-6">
                <section className="rounded-lg border border-blue-200/60 bg-blue-50/40 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-8 gap-8">
                    <FormField
                          ...
                          <FormItem className="md:col-span-2">
                            ...
                            <FormControl>
                              <Input
                                ...
                                inputMode="numeric"
                                autoComplete="off"
                                placeholder="مثال: 12 (عدد المراسي السياحية)"
                                onChange={(e) => {
                    ...
                    <div className="sm:col-span-2 xl:col-span-4 grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <FormField
                        control={form.control}
                ...
                {showFloatingRequirementsSection && (
            ...
            <>
              <div className="rounded-lg border border-blue-200/60 bg-blue-50/40 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
                <EmployeeOrganizationForm
                  key={addEmployeeKey}
                  initialData={null}
                  name="مسئول الشركة المالكة"
                  organizationId={data.id}
                  defaultPhone={form.watch("phone") ?? ""}
                  hideBackButton
                  onSuccess={async () => {
                    clearPostUpdateIdleTimer();
                    await syncOwningCompanyAfterEmployeeChange();
...
export default OwningCompaniesForm;
