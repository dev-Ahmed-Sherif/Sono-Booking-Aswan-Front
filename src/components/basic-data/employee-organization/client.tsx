...
import { EmployeeOrganizationColumn, createColumns } from "@/components/basic-data/employee-organization/columns";
...
type ClientProps = {
  data: EmployeeOrganizationColumn[] | null;
  onEditClick?: (row: EmployeeOrganizationColumn) => void;
  onDeleteSuccess?: (id: string) => void;
  showDelegateColumns?: boolean;
};

const Client = memo(({ data, onEditClick, onDeleteSuccess, showDelegateColumns = true }: ClientProps) => {
  ...
  const [tableData, setTableData] = useState<EmployeeOrganizationColumn[]>(
    data ?? [],
  );
  ...
  const columns = useMemo(
    () =>
      createColumns(onEditClick, showDelegateColumns, (id) => {
        setTableData((prev) => prev.filter((r) => r.id !== id));
        onDeleteSuccess?.(id);
      }),
    [onDeleteSuccess, onEditClick, showDelegateColumns]
  );
  return (
      ...
      <div className="p-2 sm:p-4 flex flex-col sm:flex-row items-center sm:justify-between gap-3 sm:gap-4">
        <div className="w-full sm:w-auto text-center sm:text-right">
          <Heading
            ...
            description="إدارة موظفي الجهات"
          />
        </div>
      </div>
