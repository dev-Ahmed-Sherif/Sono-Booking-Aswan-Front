...
import CityClient from "@/components/basic-data/city/client";
...
const GovernorateForm = ({
  ...
  const listBackPath = `/${locale}/basic-data/governorate`;
  ...
  const onSubmit = async (values: GovernorateFormValues) => {
    try {
      toggleLoading();
      let savedId = "";
      const fd = buildGovernorateFormData(values);
      if (data?.id) {
        const res = (await updateGovernorateMultipartById(fd)) as SaveResponse;
        if (res?.error) throw new Error(res.message || res.error);
        startPostUpdateIdleTimer();
      } else {
        const res = (await addGovernorateMultipart(fd)) as SaveResponse;
        if (res?.error) throw new Error(res.message || res.error);
