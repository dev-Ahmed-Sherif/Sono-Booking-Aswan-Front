...
import {
  OrganizationCatagoryColumn,
  createColumns,
} from "@/components/settings/organization-catagory/columns";
...
type ClientProps = {
  data: OrganizationCatagoryColumn[] | null;
  path: string;
};
...
const Client = memo(({ data, path }: ClientProps) => {
  ...
  const backPath = `/${locale}/settings`;
      ...
      <div className="p-2 sm:p-4 flex flex-col sm:flex-row items-center sm:justify-between gap-3 sm:gap-4">
        <div className="w-full sm:w-auto text-center sm:text-right">
          <Heading
            ...
            description="إدارة فئات المنظمة"
          />
        </div>
        <Button
