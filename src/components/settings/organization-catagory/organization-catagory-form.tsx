...
type OrganizationCatagoryFormProps = { initialData: any | null; name: string };
type OrganizationCatagoryFormValues = z.infer<typeof lookupSchema>;

const OrganizationCatagoryForm = ({
  initialData,
  name,
}: OrganizationCatagoryFormProps) => {
  ...
  const listBackPath = `/${locale}/settings/organization-catagory`;

  const form = useForm<OrganizationCatagoryFormValues>({
  ...
  const onSubmit = async (values: OrganizationCatagoryFormValues) => {
    try {
      toggleLoading();
      if (data?.id) {
        const res = await updateOrganizationCategoryById({
          ...values,
          id: data.id,
        ...
        const res = await addOrganizationCategory(values);
        if (res?.error) throw new Error(res.message || res.error);
      }
      router.refresh();
      setTimeout(
        () => router.push(`/${params.locale}/settings/organization-catagory`),
        1000,
      );
...
export default OrganizationCatagoryForm;
