...
import { ApartmentColumn, columns } from "@/components/settings/unit-data/columns";

type UnitDataClientProps = {
  data: ApartmentColumn[] | null;
  path: string;
};

const Client = memo(({ data, path }: UnitDataClientProps) => {
  ...
  return (
    <div className="mt-7 sm:mt-0 space-y-4">
      ...
      <div className="overflow-x-auto rounded-md">
        <DataTable searchKey="apartmentNumber" columns={columns} data={rows} />
      </div>
    </div>
  );
});

Client.displayName = "UnitDataClient";
