...
} from "@/actions/settings/inspectionTypeService";
import { ToastAction } from "@radix-ui/react-toast";

type InspectionTypesFormProps = {
  initialData: any | null;
  name: string;
  basePath?: string;
};
type EmployeesFormValues = z.infer<typeof lookupSchema>;

const InspectionTypesForm = ({
  initialData,
  name,
  basePath = "/settings/inspection-types",
}: InspectionTypesFormProps) => {
  ...
  const onSubmit = async (data: EmployeesFormValues) => {
    try {
      toggleLoading();
      if (initialData) {
        const res = await updateInspectionTypeById({
          ...data,
          id: initialData.id,
        ...
        const res = await addInspectionType(data);
        if (res?.error) throw new Error(res.message || res.error);
      }
      router.refresh();
      setTimeout(() => router.push(`/${params.locale}${basePath}`), 1000);
  ...
  const onDelete = async () => {
    try {
      ...
      toggleLoading();
      setTimeout(() => {
        router.push(`/${params.locale}${basePath}`);
        setTimeout(() => {
          router.refresh();
