...
import * as service from "@/actions/settings/paymentMethodService";
...
type PaymentMethodFormValues = z.infer<typeof lookupSchema>;
type ServiceFn = (arg: any) => Promise<any>;

const svc = service as unknown as Record<string, ServiceFn | undefined>;
const addFn = svc["addPaymentMethod"];
const updateFn = svc["updatePaymentMethodById"];
const deleteFn = svc["deletePaymentMethodById"];
const softDeleteFn = svc["softDeletePaymentMethodById"];

const PaymentMethodForm = ({ initialData, name }: PaymentMethodFormProps) => {
  ...
  const onSubmit = async (values: PaymentMethodFormValues) => {
    try {
      ...
      router.refresh();
      setTimeout(() => router.push(`/${params.locale}/settings/payment-methods`), 300);
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "حدث خطأ",
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toggleOpen();
      router.push(`/${params.locale}/settings/payment-methods`);
      router.refresh();
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  return (
    <>
      <Button
        variant="ghost"
        size="sm"
        onClick={() => router.push(`/${params.locale}/settings/payment-methods`)}
