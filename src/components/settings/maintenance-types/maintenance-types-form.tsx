...
type MaintenanceTypesFormProps = { initialData: any | null; name: string };
type MaintenanceTypesFormValues = z.infer<typeof lookupSchema>;

const MaintenanceTypesForm = ({
  initialData,
  name,
}: MaintenanceTypesFormProps) => {
  ...
  const listBackPath = `/${locale}/settings/maintenance-types`;

  const form = useForm<MaintenanceTypesFormValues>({
  ...
  const onSubmit = async (data: MaintenanceTypesFormValues) => {
    try {
      toggleLoading();
      if (initialData) {
        const res = await updateMaintenanceTypeById({
          id: initialData.id,
        ...
        const res = await addMaintenanceType(data);
        if (res?.error) throw new Error(res.message || res.error);
      }
      router.refresh();
      setTimeout(
        () => router.push(`/${params.locale}/settings/maintenance-types`),
        1000,
      );
