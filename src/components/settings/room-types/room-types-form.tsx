...
const RoomTypeForm = ({ initialData, name }: RoomTypeFormProps) => {
  ...
  const onSubmit = async (values: RoomTypeFormValues) => {
    try {
      ...
      router.refresh();
      setTimeout(() => router.push(`/${params.locale}/settings/room-types`), 300);
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "حدث خطأ",
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toggleOpen();
      router.push(`/${params.locale}/settings/room-types`);
      router.refresh();
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  return (
    <>
      <Button
        variant="ghost"
        size="sm"
        onClick={() => router.push(`/${params.locale}/settings/room-types`)}
