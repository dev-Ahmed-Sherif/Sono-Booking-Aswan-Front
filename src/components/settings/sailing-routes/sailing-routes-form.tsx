...
} from "@/actions/settings/sailingRouteService";

import { ToastAction } from "@radix-ui/react-toast";

type SailingRoutesFormProps = {
  initialData: any | null;
  name: string;
};

type SailingRoutesFormValues = z.infer<typeof lookupSchema>;

const SailingRoutesForm = ({ initialData, name }: SailingRoutesFormProps) => {
  ...
  const listBackPath = `/${locale}/settings/sailing-routes`;

  const form = useForm<SailingRoutesFormValues>({
  ...
  const onSubmit = async (data: SailingRoutesFormValues) => {
    try {
      toggleLoading();

      if (initialData) {
        const res = await updateSailingRouteById({
          ...data,
          id: initialData.id,
        ...
        const res = await addSailingRoute(data);
      ...
      router.refresh();
      setTimeout(() => {
        router.push(`/${params.locale}/settings/sailing-routes`);
      }, 1000);
...
export default SailingRoutesForm;
