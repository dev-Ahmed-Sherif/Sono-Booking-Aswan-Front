...
import * as service from "@/actions/settings/relationshipService";
...
type RelationshipFormValues = z.infer<typeof lookupSchema>;
type ServiceFn = (arg: any) => Promise<any>;

const svc = service as unknown as Record<string, ServiceFn | undefined>;
const addFn = svc["addRelationship"];
const updateFn = svc["updateRelationshipById"];
const deleteFn = svc["deleteRelationshipById"];
const softDeleteFn = svc["softDeleteRelationshipById"];

const RelationshipForm = ({ initialData, name }: RelationshipFormProps) => {
  ...
  const onSubmit = async (values: RelationshipFormValues) => {
    try {
      ...
      router.refresh();
      setTimeout(() => router.push(`/${params.locale}/settings/relationships`), 300);
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "حدث خطأ",
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toggleOpen();
      router.push(`/${params.locale}/settings/relationships`);
      router.refresh();
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  return (
    <>
      <Button
        variant="ghost"
        size="sm"
        onClick={() => router.push(`/${params.locale}/settings/relationships`)}
