...
import CellAction from "@/components/settings/apartment-types/cell-action";
import { formatUtcToCairo } from "@/lib/date-timeOptions";

export type ApartmentTypeColumn = {
  id: string;
  nameAr: string;
  nameEn: string;
  isDeleted?: boolean;
...
): ColumnDef<ApartmentTypeColumn>[] {
  const base: ColumnDef<ApartmentTypeColumn>[] = [
    { accessorKey: "nameAr", header: "الاسم العربي" },
    { accessorKey: "nameEn", header: "الاسم الإنجليزي" },
  ];

  const auditCols: ColumnDef<ApartmentTypeColumn>[] = [
    {
