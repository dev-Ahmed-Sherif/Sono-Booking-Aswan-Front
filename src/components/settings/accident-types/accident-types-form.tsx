...
} from "@/actions/settings/accidentTypeService";
import { ToastAction } from "@radix-ui/react-toast";

type AccidentTypesFormProps = { initialData: any | null; name: string };
type AccidentTypesFormValues = z.infer<typeof lookupSchema>;

const AccidentTypesForm = ({ initialData, name }: AccidentTypesFormProps) => {
  ...
  const listBackPath = `/${locale}/settings/accident-types`;

  const form = useForm<AccidentTypesFormValues>({
  ...
  const onSubmit = async (data: AccidentTypesFormValues) => {
    try {
      toggleLoading();
      if (initialData) {
        const res = await updateAccidentTypeById({
          ...data,
          id: initialData.id,
        ...
        const res = await addAccidentType(data);
        if (res?.error) throw new Error(res.message || res.error);
      }
      router.refresh();
      setTimeout(
        () => router.push(`/${params.locale}/settings/accident-types`),
        1000,
      );
...
export default AccidentTypesForm;
