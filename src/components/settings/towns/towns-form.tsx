...
import * as service from "@/actions/settings/townService";
...
type TownFormValues = z.infer<typeof lookupSchema>;
type ServiceFn = (arg: any) => Promise<any>;

const svc = service as unknown as Record<string, ServiceFn | undefined>;
const addFn = svc["addTown"];
const updateFn = svc["updateTownById"];
const deleteFn = svc["deleteTownById"];
const softDeleteFn = svc["softDeleteTownById"];

const TownForm = ({ initialData, name }: TownFormProps) => {
  ...
  const onSubmit = async (values: TownFormValues) => {
    try {
      toggleLoading();
      if (initialData?.id) {
      ...
      router.refresh();
      setTimeout(() => router.push(`/${params.locale}/settings/towns`), 300);
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "حدث خطأ",
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toggleOpen();
      router.push(`/${params.locale}/settings/towns`);
      router.refresh();
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  return (
    <>
      <Button
        variant="ghost"
        size="sm"
        onClick={() => router.push(`/${params.locale}/settings/towns`)}
