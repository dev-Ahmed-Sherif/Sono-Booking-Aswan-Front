...
import * as service from "@/actions/settings/requestTypeService";
...
type RequestTypeFormValues = z.infer<typeof lookupSchema>;
type ServiceFn = (arg: any) => Promise<any>;

const svc = service as unknown as Record<string, ServiceFn | undefined>;
const addFn = svc["addRequestType"];
const updateFn = svc["updateRequestTypeById"];
const deleteFn = svc["deleteRequestTypeById"];
const softDeleteFn = svc["softDeleteRequestTypeById"];

const RequestTypeForm = ({ initialData, name }: RequestTypeFormProps) => {
  ...
  const onSubmit = async (values: RequestTypeFormValues) => {
    try {
      ...
      router.refresh();
      setTimeout(() => router.push(`/${params.locale}/settings/request-types`), 300);
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "حدث خطأ",
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      toggleOpen();
      router.push(`/${params.locale}/settings/request-types`);
      router.refresh();
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  return (
    <>
      <Button
        variant="ghost"
        size="sm"
        onClick={() => router.push(`/${params.locale}/settings/request-types`)}
