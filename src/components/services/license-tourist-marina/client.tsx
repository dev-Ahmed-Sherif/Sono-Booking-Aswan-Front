...
import {
  LicenseTouristMarinaColumn,
  createColumns,
} from "@/components/services/license-tourist-marina/columns";
...
type ClientProps = {
  data: LicenseTouristMarinaColumn[] | null;
  path: string;
};
...
const Client = memo(({ data, path }: ClientProps) => {
  ...
  const backPath = `/${locale}/services`;
      ...
      <div className="p-2 sm:p-4 flex flex-col sm:flex-row items-center sm:justify-between gap-3 sm:gap-4">
        <div className="w-full sm:w-auto text-center sm:text-right">
          <Heading
            ...
            description="طلبات ترخيص المراسي السياحية"
          />
        </div>
        <Button
