...
import {
  MaintenanceColumn,
  createColumns,
} from "@/components/operation/maintenance/columns";
...
const Client = memo(({ data, path }: ClientProps) => {
  ...
  const backPath = `/${locale}/operation`;
  const isSuperAdmin = useMemo(
    () => isSuperAdminRoleCandidates(user.getItem() as RoleCandidates),
    [user],
  );
  const columns = useMemo(() => createColumns(isSuperAdmin), [isSuperAdmin]);
  ...
  return (
    <div className="mt-7 sm:mt-0 space-y-4">
      ...
      <div className="p-2 sm:p-4 flex flex-col sm:flex-row items-center sm:justify-between gap-3 sm:gap-4">
        <div className="w-full sm:w-auto text-center sm:text-right">
          <Heading
            ...
            description="إدارة عمليات صيانة العائمات"
          />
        </div>
        <Button
      ...
      <div className="overflow-x-auto rounded-md">
        ...
        <DataTable searchKey="number" columns={columns} data={tableData} />
      </div>
    </div>
  );
});
